...
const RecruitingItem: FC<RecruitingItemProps> = ({ recruiting, onClick, isActive }) => {
    ...
    return (
        <div onClick={handleClick} className={cn("p-4 border-b border-gray-100 hover:bg-gray-50 cursor-pointer", {
            ...
            <div className="flex items-start space-x-3">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center">
                        ...
                        {recruiting?.lastMessage?.createdAt && <span className="text-xs text-gray-500">{formatTime(String(recruiting?.lastMessage?.createdAt))}</span>}
                    </div>
                    <p className="text-sm text-gray-500 line-clamp-1">{recruiting?.job?.recruiter?.firstName}</p>
                    <div className={cn('grid gap-x-1 items-center mt-1', {
                        'grid-cols-[20px_1fr]': recruiting?.lastMessage?.content
                    })}>
                        <div className="text-sm translate-y-[1px]">
                            {recruiting?.lastMessage?.content &&
                                <FaArrowRightLong className={cn('text-blue-400', {
                                    'rotate-180 text-green-400': recruiting?.lastMessage?.senderType === SENDER_TYPE.RECRUITER
                                })} />
                            }
                        </div>
