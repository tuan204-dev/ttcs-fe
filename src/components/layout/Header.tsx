...
import { FiBookmark, FiHome, FiMessageSquare, FiUser } from 'react-icons/fi';
...
const Header = () => {
    ...
    return (
        <header className="h-16 fixed top-0 left-0 right-0 bg-white border-b border-gray-200 z-10 pl-5 flex justify-between items-center">
            <MenuDrawer isOpen={isOpenMenu} onClose={() => setIsOpenMenu(false)} />
            <Link href={'/'}>
                <h1 className="text-3xl font-bold text-blue-600">RecruitPro</h1>
            </Link>
            <div className="flex items-center gap-x-3 h-full">
                {menus.map((item) => (
                    <Link
                        key={item.path}
                        href={item.path}
                        className={cn('flex items-center gap-x-3 h-full px-3 cursor-pointer text-gray-950', {
                ...
                <button onClick={() => setIsOpenMenu(true)} className="text-2xl px-4">
                    <RxHamburgerMenu />
                </button>
            </div>
        </header>
